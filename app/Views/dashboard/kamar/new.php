...
<div class="bg-white rounded shadow">
    ...
    <div class="block p-4">
        <form action="<?= base_url('dashboard/kamar/create') ?>" method="post">
            <div class="flex flex-wrap -mx-3 mb-6">
                ...
                <div class="w-full px-3 mb-6 md:mb-0">
                    ...
                    <input class="form-control" id="kapasitas" type="number" name="kapasitas" value="<?= old('kapasitas') ?>" required>
                </div>
                <div class="w-full px-3 mb-6 md:mb-0">
                    <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="daerah">
                        Daerah
                    </label>
                    <input class="form-control" id="daerah" type="text" name="daerah" value="<?= old('daerah') ?>" required>
                </div>
                <div class="w-full px-3 mb-6 md:mb-0">
                    ...
                    <?php if(count($dokter) > 0): ?>
                        <select name="dokter_id" id="dokter_id" class="!block" required>
                            <?php foreach($dokter as $d): ?>
                                <option value="<?= $d->id ?>" <?= old('dokter_id') == $d->id ? 'selected' : '' ?>><?= $d->nama ?> (<?= $d->bidang_keahlian ?>)</option>
                            <?php endforeach; ?>
                        </select>
                    <?php else: ?>
                        <p>
